...
format 'hhmm'.  I will therefore process the interval field into time objects from
the 'chron' package.
...
```{r daily_totals}
day_totals <- function(activity) {
    return(activity %>% group_by(date) %>% na.omit %>% summarise(total = sum(steps)))
    }


daily_totals <- day_totals(activity)
```
...
if (anyNA(activity$steps)) {
    ...
    return("All NA values have been successfully replaced")
    }
}
